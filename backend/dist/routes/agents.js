...
agents.get('/', async (c) => {
    try {
        ...
        if (payload.role === 'admin') {
            result = await c.env.DB.prepare('SELECT * FROM agents ORDER BY sort_order ASC, created_at DESC').all();
        }
        else {
            result = await c.env.DB.prepare('SELECT * FROM agents WHERE created_by = ? ORDER BY sort_order ASC, created_at DESC').bind(payload.id).all();
        }
        const agents = (result.results || []).map((a) => {
            const { ip_address: _ip, ...restNoIp } = a;
            if (payload.role !== 'admin') {
                const { token } = restNoIp;
                return restNoIp;
            }
            return restNoIp;
        });
...
agents.post('/', async (c) => {
    try {
        ...
        if (!computedExpiry && start_time && duration_value && duration_unit) {
            computedExpiry = (0, jwt_2.addDuration)(new Date(start_time), duration_value, duration_unit).toISOString();
        }
        // 插入新客户端
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(name, token, payload.id, 'inactive', category || null, tags || null, isPublic !== undefined ? (isPublic ? 1 : 0) : 1, computedExpiry, start_time || null, duration_value || null, duration_unit || null, traffic_limit || null, now, now).run();
        if (!result.success) {
...
agents.get('/tags/pool', async (c) => {
    try {
        ...
        let result;
        if (payload.role === 'admin') {
            result = await c.env.DB.prepare("SELECT tags FROM agents WHERE tags IS NOT NULL AND tags != ''").all();
        }
        else {
            result = await c.env.DB.prepare("SELECT tags FROM agents WHERE created_by = ? AND tags IS NOT NULL AND tags != ''").bind(payload.id).all();
        }
...
agents.get('/groups', async (c) => {
    try {
        const result = await c.env.DB.prepare(`SELECT g.*, (SELECT COUNT(*) FROM agents WHERE category = g.name) as agent_count
       FROM agent_groups g ORDER BY g.name ASC`).all();
...
agents.post('/groups', async (c) => {
    try {
        ...
        if (!name?.trim())
            return c.json({ success: false, message: '分组名不能为空' }, 400);
        const now = new Date().toISOString();
        ...
        if (e.message?.includes('UNIQUE'))
            return c.json({ success: false, message: '分组名已存在' }, 409);
...
agents.post('/groups/:id/agents', async (c) => {
    try {
        ...
        const group = await c.env.DB.prepare('SELECT name FROM agent_groups WHERE id = ?').bind(groupId).first();
        if (!group)
...
agents.delete('/groups/:id', async (c) => {
    try {
        ...
        const group = await c.env.DB.prepare('SELECT name FROM agent_groups WHERE id = ?').bind(id).first();
        if (!group)
            return c.json({ success: false, message: '分组不存在' }, 404);
        ...
        if (!result.success)
            throw new Error('删除失败: ' + (result.error || 'unknown'));
...
agents.get('/groups/pool', async (c) => {
    try {
        const result = await c.env.DB.prepare('SELECT name FROM agent_groups ORDER BY name ASC').all();
...
agents.post('/reorder', async (c) => {
    try {
        ...
        const all = await c.env.DB.prepare('SELECT id, sort_order FROM agents ORDER BY sort_order ASC, created_at DESC').all();
    ...
    try {
        ...
        return c.json({
            ...
            agent: {
                ...
                process_count: agent.process_count ?? null,
                tcp_count: agent.tcp_count ?? null,
                udp_count: agent.udp_count ?? null,
...
agents.get('/:id/metrics', async (c) => {
    try {
        ...
        const rows = c.env.DB.prepare('SELECT timestamp, cpu, mem_pct, disk_pct, net_rx, net_tx, process_count, tcp_count, udp_count FROM agent_metrics_history WHERE agent_id = ? AND timestamp >= ? ORDER BY timestamp ASC').bind(agentId, since).all();
        return c.json({
            success: true,
            metrics: (rows.results || []).map((r) => ({
                ts: r.timestamp,
                cpu: r.cpu ?? 0,
                mem: r.mem_pct ?? 0,
                disk: r.disk_pct ?? 0,
...
agents.put('/:id', async (c) => {
    try {
        ...
        if (start_time !== undefined || duration_value !== undefined || duration_unit !== undefined) {
            ...
            if (st && dv && du) {
                const recomputedExpiry = (0, jwt_2.addDuration)(new Date(st), dv, du).toISOString();
...
agents.post('/:id/status', async (c) => {
    try {
       ...
       WHERE id = ?`).bind(cpu_usage, memory_total, memory_used, disk_total, disk_used, network_rx, network_tx, hostname, ip_address, os, version, new Date().toISOString(), agentId).run();
        ...
        const agentForRenew = await c.env.DB.prepare('SELECT expiry_time, duration_value, duration_unit FROM agents WHERE id = ?').bind(agentId).first();
        if (agentForRenew?.expiry_time && agentForRenew?.duration_value && agentForRenew?.duration_unit) {
            ...
            if (now > new Date(agentForRenew.expiry_time)) {
                const newExpiry = (0, jwt_2.addDuration)(now, agentForRenew.duration_value, agentForRenew.duration_unit);
                await c.env.DB.prepare('UPDATE agents SET start_time = ?, expiry_time = ? WHERE id = ?').bind(now.toISOString(), newExpiry.toISOString(), agentId).run();
            }
        }
        return c.json({
            success: true,
...
agents.delete('/:id', async (c) => {
    try {
        ...
        for (const sql of [
            ...
            try {
                await c.env.DB.prepare(sql).bind(agent.id).run();
            }
            catch (e) { /* 表可能不存在，忽略 */ }
        }
        // 执行删除客户端
...
agents.post('/register', async (c) => {
    try {
        ...
        const autoName = (0, jwt_2.generateAgentName)(null);
        const insertResult = await c.env.DB.prepare(`INSERT INTO agents (name, token, created_by, status, hostname, ip_address, os, version, created_at, updated_at)
       VALUES (?, ?, ?, 'active', ?, ?, ?, ?, ?, ?)`).bind(autoName, token, adminUser.id, hostname || null, ip_address || null, os || null, version || null, now, now).run();
        if (!insertResult.success) {
            throw new Error('自动创建客户端失败: ' + (insertResult.error || 'unknown'));
        }
        const created = await c.env.DB.prepare('SELECT * FROM agents WHERE rowid = last_insert_rowid()').first();
    ...
    try {
       ...
       WHERE id = ?`).bind(cpu_usage, memory_total, memory_used, disk_total, disk_used, network_rx, network_tx, (0, jwt_2.toD1Primitive)(hostname), (0, jwt_2.toD1Primitive)(ip_address), (0, jwt_2.toD1Primitive)(os), (0, jwt_2.toD1Primitive)(version), new Date().toISOString(), agent.id).run();
        ...
        if (agentForRenew?.expiry_time && agentForRenew?.duration_value && agentForRenew?.duration_unit) {
            ...
            if (now > new Date(agentForRenew.expiry_time)) {
                const newExpiry = (0, jwt_2.addDuration)(now, agentForRenew.duration_value, agentForRenew.duration_unit);
                await c.env.DB.prepare('UPDATE agents SET start_time = ?, expiry_time = ? WHERE id = ?').bind(now.toISOString(), newExpiry.toISOString(), agent.id).run();
            }
        }
        return c.json({
            success: true,
