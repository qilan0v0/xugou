...
exports.addSampleMonitors = addSampleMonitors;
exports.addSampleAgents = addSampleAgents;
...
async function createAdminUser(env) {
    console.log('检查管理员用户...');
    const adminUser = await env.DB.prepare('SELECT id FROM users WHERE username = ?').bind('admin').first();
    // 如果不存在管理员用户，则创建一个
    if (!adminUser) {
        console.log('创建管理员用户...');
        // 密码: admin123
...
async function addSampleMonitors(env) {
    ...
    if (existingMonitors.count === 0) {
        ...
        await env.DB.prepare(`INSERT INTO monitors (name, url, method, interval, timeout, expected_status, headers, body, created_by, active, status, uptime, response_time, last_checked, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind('百度', 'https://www.baidu.com', 'GET', 60, 30, 200, '{}', '', userId, true, 'up', 99.98, 120, now, now, now).run();
        await env.DB.prepare(`INSERT INTO monitors (name, url, method, interval, timeout, expected_status, headers, body, created_by, active, status, uptime, response_time, last_checked, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind('哔哩哔哩', 'https://www.bilibili.com', 'GET', 30, 30, 200, '{}', '', userId, true, 'up', 99.95, 150, now, now, now).run();
        await env.DB.prepare(`INSERT INTO monitors (name, url, method, interval, timeout, expected_status, headers, body, created_by, active, status, uptime, response_time, last_checked, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind('油管', 'https://www.youtube.com', 'GET', 60, 30, 200, '{}', '', userId, true, 'up', 99.9, 180, now, now, now).run();
    }
}
// 添加示例客户端数据
async function addSampleAgents(env) {
    ...
    if (existingAgents.count === 0) {
        ...
        const userId = 1; // 管理员用户ID
        // 主服务器
        await env.DB.prepare(`INSERT INTO agents (name, token, created_by, status, created_at, updated_at, cpu_usage, memory_total, memory_used, disk_total, disk_used, network_rx, network_tx, hostname, ip_address, os, version)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind('主服务器', 'primary-server-token-123456', userId, 'active', now, now, 25.5, // CPU使用率
        ...
        // 备份服务器
        await env.DB.prepare(`INSERT INTO agents (name, token, created_by, status, created_at, updated_at, cpu_usage, memory_total, memory_used, disk_total, disk_used, network_rx, network_tx, hostname, ip_address, os, version)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind('备份服务器', 'backup-server-token-789012', userId, 'active', now, now, 15.0, // CPU使用率
        8192, // 内存总量 (MB)
        4096, // 内存使用量 (MB)
        1000000, // 磁盘总量 (MB)
        300000, // 磁盘使用量 (MB)
        ...
        // 应用服务器
        await env.DB.prepare(`INSERT INTO agents (name, token, created_by, status, created_at, updated_at, cpu_usage, memory_total, memory_used, disk_total, disk_used, network_rx, network_tx, hostname, ip_address, os, version)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind('应用服务器', 'app-server-token-345678', userId, 'active', now, now, 45.0, // CPU使用率
        32768, // 内存总量 (MB)
        24576, // 内存使用量 (MB)
        250000, // 磁盘总量 (MB)
        150000, // 磁盘使用量 (MB)
...
// 创建默认状态页配置
async function createDefaultStatusPage(env) {
    // 检查是否已有状态页配置
    const existingConfig = await env.DB.prepare('SELECT COUNT(*) as count FROM status_page_config').first();
    if (existingConfig && existingConfig.count === 0) {
        console.log('创建默认状态页配置...');
        const now = new Date().toISOString();
        const userId = 1; // 管理员用户ID
        // 创建配置
        const result = await env.DB.prepare(`INSERT INTO status_page_config (user_id, title, description, logo_url, custom_css, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(userId, '系统状态', '实时监控系统运行状态', '', '', now, now).run();
        // 获取配置ID
        const configId = await env.DB.prepare('SELECT last_insert_rowid() as id').first();
        if (configId && configId.id) {
            ...
            if (monitors.results) {
                for (const monitor of monitors.results) {
                    await env.DB.prepare('INSERT INTO status_page_monitors (config_id, monitor_id) VALUES (?, ?)').bind(configId.id, monitor.id).run();
                }
            }
            // 关联所有客户端
            ...
            if (agents.results) {
                for (const agent of agents.results) {
                    await env.DB.prepare('INSERT INTO status_page_agents (config_id, agent_id) VALUES (?, ?)').bind(configId.id, agent.id).run();
                }
            }
        }
    }
