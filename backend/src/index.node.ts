...
app.post('/api/agents/status', async (c) => {
  try {
    ...
    const prev = await env.DB.prepare('SELECT status, updated_at, connected_at, boot_time FROM agents WHERE id = ?').bind(agent.id).first() as {status: string; updated_at: string; connected_at: string | null; boot_time: string | null} | null;
    ...
    // 重置时机：缺失 / 之前被标记离线(status=inactive) / 上报中断过久 / 系统重启(boot_time变化)
    ...
    console.log(`[conn] agent=${agent.id} status=${currentStatus || '?'} gap=${gapSec}s old_conn=${prev?.connected_at || 'null'} boot_chg=${!!bootChanged} -> reset=${shouldReset} new_conn=${newConnectedAt}`);
...
app.post('/api/v1/agent/report', async (c) => {
  try {
    ...
    if (!token) return c.json({ success: false, message: 'no client_secret' }, 400);
    ...
    const version = toD1Primitive(host.platform_version ?? null);
    const hostname = toD1Primitive(host.hostname ?? null);
    ...
    // IP 地址
    const ipAddress = toD1Primitive(
      body.ip ?? (Array.isArray(host.ip) ? host.ip[0] : host.ip) ?? null
    );
    ...
    const result = env.DB.prepare(
      ...
      country,
      now, raw.slice(0, 2000),
      processCount, tcpCount, udpCount,
      agent.id
    ).run();
    ...
    try {
      ...
      env.DB.prepare(
        'INSERT INTO agent_metrics_history (agent_id, timestamp, cpu, mem_pct, disk_pct, net_rx, net_tx, process_count, tcp_count, udp_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
      ).bind(agent.id, now, cpu, memPctVal, diskPctVal, netRx, netTx, processCount, tcpCount, udpCount).run();
      env.DB.prepare(
        'DELETE FROM agent_metrics_history WHERE agent_id = ? AND id NOT IN (SELECT id FROM agent_metrics_history WHERE agent_id = ? ORDER BY id DESC LIMIT 2880)'
      ).bind(agent.id, agent.id).run();
    } catch (e) { /* non-critical */ }
