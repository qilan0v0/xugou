...
function escapeMarkdownLegacy(s: string): string {
...
export interface TemplateOptions {
  json?: boolean; // 模板是 JSON（默认 true）
}

export function applyTemplate(template: string, vars: Record<string, string>, opts: TemplateOptions = {}): string {
  ...
  for (const [k, v] of Object.entries(vars)) {
    ...
    if (mdV2) val = escapeMarkdownV2(val);
    else if (mdLegacy) val = escapeMarkdownLegacy(val);
    if (json) val = escapeJsonString(val);
