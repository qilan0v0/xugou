...
import { getAgent, Agent } from '../api/agents';
...
export default function AgentDetailModal({ agentId, onClose }: AgentDetailModalProps) {
  ...
  const fetchAgent = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && res.agent) setAgent(res.agent);
    } catch { /* ignore */ }
  ...
  const mem = agent && agent.memory_total && agent.memory_used ? Math.round((agent.memory_used / agent.memory_total) * 100) : 0;
  const disk = agent && agent.disk_total && agent.disk_used ? Math.round((agent.disk_used / agent.disk_total) * 100) : 0;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto glass rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {loading ? (
          ...
          <div className="flex justify-center items-center min-h-[200px]">
            <span className="text-slate-500 text-sm">获取客户端信息失败</span>
          </div>
        ) : (
          <div className="p-6">
            {/* Header */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="glass rounded-xl p-4">
                <h3 className="font-semibold text-sm text-slate-900 dark:text-white mb-3">系统资源</h3>
            ...
            <div className="flex justify-end mt-4 pt-3 border-t border-white/[0.06]">
              <button onClick={fetchAgent} className="px-3 py-1.5 rounded-lg text-xs text-slate-500 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors flex items-center gap-1">
                <CodeIcon className="w-3 h-3" />刷新
              </button>
            </div>
          </div>
        )}
