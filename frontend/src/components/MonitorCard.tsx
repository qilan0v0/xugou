...
const MonitorCard = React.memo(({ monitor, onClick }: MonitorCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className={`shrink-0 min-w-[52px] text-center text-[11px] font-semibold text-white rounded-full px-2 py-0.5 leading-5 ${uptimeColor}`}>
          ...
          {monitor.tags && (
            <div className="flex items-center gap-1 flex-wrap">
              {monitor.tags.split(',').filter(Boolean).map((tag: string, i: number) => {
                ...
                return (
                  <span key={i} className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${c}`}>
                    {t}
                  </span>
                );
              })}
