...
const StatusPageConfig = () => {
  ...
  const inputClass = "w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-300 placeholder:text-slate-500 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 transition-all";
                    ...
                    {varsExpanded && (
                      <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] bg-slate-50 dark:bg-white/[0.03] rounded-lg p-3 border border-white/[0.06]">
                        {[
                          ...
                          ['{boot_time}', '系统启动时间'],
                          ['{connected_at}', '首次连接时间'],
                          ['{network_rx}', '实时下载速率 (仅客户端)'],
                          ['{network_tx}', '实时上传速率 (仅客户端)'],
                          ['{network_rx_total}', '总下载流量'],
                          ['{network_tx_total}', '总上传流量'],
                          ['{traffic_total}', '总流量（下载+上传）'],
                          ['{process_count}', '进程数 (仅客户端)'],
                          ['{tcp_count}', 'TCP 连接数 (仅客户端)'],
                          ['{udp_count}', 'UDP 连接数 (仅客户端)'],
                          ['{message}', '故障/恢复 描述'],
                          ['{url}', '监控URL (仅API监控)'],
                          ['{method}', '请求方法 (仅API监控)'],
